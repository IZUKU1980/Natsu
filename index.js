...
async function connectToWhatsApp() {
    ...
    if (!rich.authState.creds.registered) {
        const phoneNumber = await question(console.log("Veuillez saisir votre numéro WhatsApp. Example : 509xxxxxxxxx"));
...
rich.ev.on("connection.update", async (update) => {
        ...
        if (connection === "open") {
            console.log(chalk.green(`✅ Connecté à WhatsApp en tant que ${rich.user.id}`));
        } else if (connection === "close" && lastDisconnect?.error?.output?.statusCode !== 401) {
            console.log(chalk.red("⚠️ Connexion fermée, reconnexion en cours..."));
            connectToWhatsApp();
        }
    });   
